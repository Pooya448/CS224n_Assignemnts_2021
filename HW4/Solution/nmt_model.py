...
class NMT(nn.Module):
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1, dim=0):
            sq = torch.squeeze(Y_t, dim=0)
            Ybar_t = torch.cat((sq, o_prev), dim=1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden_unsq = torch.unsqueeze(dec_hidden, dim=-1)
        batch_mm = torch.bmm(enc_hiddens_proj, dec_hidden_unsq)
        e_t = torch.squeeze(batch_mm, dim=-1)
        ### END YOUR CODE
        ...
        alpha_t_unsq = torch.unsqueeze(alpha_t, dim=1)
        batch_mm = torch.bmm(alpha_t_unsq, enc_hiddens)
        a_t = torch.squeeze(batch_mm, dim=1)

        U_t = torch.cat((dec_hidden, a_t), dim=1)

        V_t = self.combined_output_projection(U_t)

        V_t_tanh = torch.tanh(V_t)
        O_t = self.dropout(V_t_tanh)
        ### END YOUR CODE
