...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tr_size = random.randint(4, int(self.block_size * 7 / 8))

        document_tr = document[:tr_size]
        len_tr = len(document_tr)

        len_mean = len_tr / 4
        noise = random.uniform(-1/8, 1/8) * len_tr

        len_mask = len_mean + noise
        len_mask = int(np.clip(len_mask, 1, len_tr - 2))

        start = random.randint(1, len_tr - len_mask - 1)

        prefix = document_tr[:start]
        masked_content = document_tr[start : start + len_mask]
        suffix = document_tr[start + len_mask : ]

        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content

        len_pad = self.block_size - len(masked_string)
        masked_string = masked_string + len_pad * self.PAD_CHAR

        input_str = masked_string[ : -1]
        output_str = masked_string[1 : ]

        xl = [self.stoi[char] for char in input_str]
        yl = [self.stoi[char] for char in output_str]

        X = torch.tensor(xl, dtype=torch.long)
